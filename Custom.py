...
def get_search_chain(name, set, personality, line, situation): # 인격을 지정하기 위해 필요한 데이터를 가져오는 코드
    ...
    search_chain = TransformChain(input_variables=["chat"], output_variables=["name", "set", "personality", "line", "situation"], transform=get_data)
    return search_chain
...
def get_chatgpt_chain(): # GPT-4를 사용하여 대화를 생성하는 코드
    ...
    prompt_template = PromptTemplate(input_variables=["chat", "current_chat_history", "name", "set", "personality", "line", "situation"], template=template)
